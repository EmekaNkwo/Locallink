...
LocalLink is a **digital walkie-talkie** for environments with no cellular signal or internet. It enables **end-to-end encrypted audio and text** directly between nearby phones on the same local network, with **no servers** in the media path.
...
- **One-touch discovery & pairing** — auto-scan for nearby LocalLink devices on the same Wi-Fi/LAN, confirm with a 6-digit code (SAS) to block man-in-the-middle.
...
```
Wi-Fi/LAN scan      →  find LocalLink discovery responders on the subnet
        ↓
LAN socket (TCP)    →  exchange peer address + serverless WebRTC signaling
        ↓
...
WebRTC normally needs a signaling server; LocalLink performs the handshake over direct LAN sockets instead. Discovery scans the current `/24` subnet for LocalLink responders, then signaling and media stay on the local network.
...
| Discovery | `react-native-tcp-socket` (Wi-Fi/LAN subnet discovery) |
...
src/
  ...
  constants/    # Theme + local-network port config
  lib/          # Byte/base64 helpers, crypto polyfill
  types/        # Shared types
```

Each service has a `*.types.ts` interface, a native implementation, and a `*.web.ts` mock — so the **web build runs entirely on mocks** for fast UI iteration, while native uses the real TCP/WebRTC/crypto stack.
...
### Native build (real audio, local networking, crypto)

The core features (WebRTC, TCP sockets, crypto) are native modules and require a dev build on physical devices. Test with **two phones on the same Wi-Fi network or one phone hotspot**:
...
- **Phase 1 — Short range (same Wi-Fi/LAN):** discovery, identity/SAS handshake, P2P audio, and messaging implemented behind stable service interfaces. Pending on-device bring-up.
